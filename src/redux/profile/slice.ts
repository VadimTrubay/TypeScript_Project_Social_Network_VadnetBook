...
const handleFetchMeProfileFulfilled = (
  state: initialProfileType,
  action: PayloadAction<any>,
) => {
  state.loading = false;
...
const handleSetMeStatusFulfilled = (
  state: initialProfileType,
  action: PayloadAction<any>,
) => {
  state.loading = false;
...
const handleEditProfileFulfilled = (
  state: initialProfileType,
  action: PayloadAction<any>,
) => {
  state.loading = false;
  ...
  state.profile.user.first_name =
    action.payload.first_name || state.profile.user.first_name;
  state.profile.user.last_name =
    action.payload.last_name || state.profile.user.last_name;
  state.profile.about_me = action.payload.about_me || state.profile.about_me;
  state.profile.website_page =
    action.payload.website_page || state.profile.website_page;
  state.profile.github_page =
    action.payload.github_page || state.profile.github_page;
  state.profile.linkedin_page =
    action.payload.linkedin_page || state.profile.linkedin_page;
  state.profile.looking_from_job =
    action.payload.looking_from_job ?? state.profile.looking_from_job;
  state.profile.job_skills =
    action.payload.job_skills || state.profile.job_skills;
  state.profile.birth_date =
    action.payload.birth_date || state.profile.birth_date;
  state.profile.phone_number =
    action.payload.phone_number || state.profile.phone_number;
  // @ts-ignore
  toast.info("Profile edited successfully", toast_settings);
};

const handleDeleteProfileFulfilled = (
  state: initialProfileType,
  action: PayloadAction<any>,
) => {
  state.loading = false;
