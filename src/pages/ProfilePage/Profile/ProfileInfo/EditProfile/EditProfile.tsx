...
const EditProfile = ({profile, setEditeProfile}) => {
  ...
  return <div className={styles.modalWrapper}>
    <div className={styles.modalWrapperGrayOpacity} onClick={() => setEditeProfile(false)}></div>
      ...
      onSubmit={(formData) => {
        dispatch(editProfile({
          user: {
            ...
            first_name: formData.first_name === "" ? null : formData.first_name,
            last_name: formData.last_name === "" ? null : formData.last_name,
          },
          about_me: formData.about_me === "" ? null : formData.about_me,
          website_page: formData.website_page === "" ? null : formData.website_page,
          github_page: formData.github_page === "" ? null : formData.github_page,
          linkedin_page: formData.linkedin_page === "" ? null : formData.linkedin_page,
          looking_from_job: formData.looking_from_job === null ? false : formData.looking_from_job,
          job_skills: formData.job_skills === "" ? null : formData.job_skills,
      ...
      {({errors, dirty, isValid}) => {
        return <Form className={styles.formzindex}>
          <div className={styles.modalBlock}>
            <button className={styles.closeButton} onClick={() => setEditeProfile(false)} type="button"></button>
            <div className={styles.leftBlock}>
              <span className={styles.modalBlockSubtitle}>General info:</span>

              {createFormikTextField('username', 'Your username', 'Change your username', errors)}
              {createFormikTextField('first_name', 'Your first name', 'Change your first name', errors)}
              {createFormikTextField('last_name', 'Your last name', 'Change your last name', errors)}
              {createFormikTextField('about_me', 'About Me', 'Change about me info', errors)}
              {createFormikTextField('birth_date', 'Birth date', 'Change about me info', errors)}
              {createFormikTextField('phone_number', 'Phone number', 'Change phone number', errors)}
            </div>

            <div className={styles.rightContactsBlock}>
              <span className={styles.modalBlockSubtitle}>Contacts:</span>
              {createFormikTextField('website_page', 'Website', 'your website', errors)}
              {createFormikTextField('github_page', 'Github', 'your github account', errors)}
              {createFormikTextField('linkedin_page', 'Linkedin', 'your linkedin account', errors)}

              <span className={styles.modalBlockSubtitle}>Job status:</span>
              {createFormikTextField('job_skills', 'Job skills', 'select search job description', errors)}
              <div className={styles.inputWrapper + ' ' + styles.checkboxWrapper}>
                <label htmlFor="looking_from_job" className={styles.checkboxLabel}>looking for a job</label>
                <Field className={styles.checkbox} id="looking_from_job" name="looking_from_job" type="checkbox"/>
              </div>
            </div>
