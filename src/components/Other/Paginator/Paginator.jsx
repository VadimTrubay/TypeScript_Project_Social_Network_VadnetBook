...
const Paginator = ({totalCount, pageSize, currentPage, onPageChanged, paginationWrapperFirstForCss}) => {
  ...
  return (
    <div className={`${styles.paginationWrapper} ${paginationWrapperFirstForCss ? styles.paginationWrapperFirst : ''}`}>
      {currentPage !== 1 ? <span className={styles.firstPageButton} onClick={() => {
        onPageChanged(1)
      }}>В начало</span> : ''}

      {pages.map(pageNumber => <span key={pageNumber} className={currentPage === pageNumber ? styles.selectedPage : ''}
                                     onClick={() => {
                                       onPageChanged(pageNumber)
                                     }}>{pageNumber}</span>)}

      {currentPage !== pagesCount ? <span className={styles.lastPageButton} onClick={() => {
